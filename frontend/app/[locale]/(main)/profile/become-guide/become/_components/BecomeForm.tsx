...
export const BecomeForm = () => {
	...
	useEffect(() => {
		logger.debug(becomeGuide.isSuccess)
						...
						<FormItem className='w-full'>
							...
							>
								...
								<SelectContent ref={field.ref}>
									{countryListQuery.isLoading && <Typography variant='content1'>Загрузка стран...</Typography>}
									{countryListQuery.isFetched && !countryListQuery.data?.length && (
										<Typography variant='content1'>Не удалось загрузить страны</Typography>
									)}
						...
						render={({ field }) => (
							<FormItem className='w-full'>
								...
								>
									...
									<SelectContent ref={field.ref}>
										{cityListQuery.isLoading && <Typography variant='content1'>Загрузка городов...</Typography>}
										{cityListQuery.isFetched && !cityListQuery.data?.length && (
											<Typography variant='content1'>Не удалось загрузить города</Typography>
										)}
