...
export const Header: FC<IHeader> = ({ tourId }) => {
  ...
  return (
    <div
      className={cn(
        "fixed left-0 right-0 top-0 py-2 transition-all duration-500",
        scrollable ? "bg-gray-200/60 backdrop-blur-2xl" : "bg-white/0 backdrop-blur-0",
      )}
    >
      <Container className="flex items-center justify-between">
        <IconButton className="rounded-full bg-white p-1" icon="ChevronLeft" description="Назад" onClick={onBack} />
        <div className="flex items-center gap-2">
          <IconButton
            ...
            onClick={onWishList}
          />
          <ShareButton shareData={{ url: locationHref }} description="Поделиться экскурсией" />
        </div>
      </Container>
    </div>
  );
