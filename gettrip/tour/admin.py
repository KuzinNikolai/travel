...
class OrderAdmin(admin.ModelAdmin):
    list_display = ('order_number', 'tour', 'full_name', 'quantity_adults', 'trip_date', 'total_price', 'deposit', 'cash_on_tour_display')     
    readonly_fields = ('order_number', 'total_price_display', 'cash_on_tour_display')

    fieldsets = (
        (None, {
            'fields': ('order_number', 'full_name', 'email', 'phone')
        }),
        ('Tour Details', {
            'fields': ('tour', 'program', 'hotel', 'room_number', 'pickup_time', 'trip_date', 'user')
        }),
        ('Count of costumers', {
            'fields': ('quantity_adults', 'quantity_children', 'quantity_infant')
        }),
        ('Financial', {
            'fields': ('transfer', 'total_price_display', 'deposit', 'cash_on_tour_display', 'text')
        }),
    )
