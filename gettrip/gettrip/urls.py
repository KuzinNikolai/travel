...
urlpatterns = [
    ...
    path('api/v1/users/<int:pk>/', CustomUserViewSet.as_view({'get': 'retrieve', 'put': 'update', 'patch': 'partial_update', 'delete': 'destroy'}), name='user-detail'), # аккаунт пользователя
    path('api/v1/users/update/<int:pk>/', UserUpdateView.as_view(), name='user-update'), # редактирование пользователя
